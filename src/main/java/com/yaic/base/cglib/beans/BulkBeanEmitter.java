...
class BulkBeanEmitter extends ClassEmitter
{
	...
	private static final Type BULK_BEAN = TypeUtils.parseType("com.yaic.base.cglib.beans.BulkBean");
	private static final Type BULK_BEAN_EXCEPTION = TypeUtils.parseType("com.yaic.base.cglib.beans.BulkBeanException");

	public BulkBeanEmitter(final ClassVisitor v, final String className, final Class<?> target, final String[] getterNames,
			final String[] setterNames, final Class[] types)
	{
		super(v);
	...
	{
		final CodeEmitter e = begin_method(Constants.ACC_PUBLIC, GET_PROPERTY_VALUES, null);
		if (getters.length > 0)
		{
			e.load_arg(0);
	...
	{
		// setPropertyValues
		final CodeEmitter e = begin_method(Constants.ACC_PUBLIC, SET_PROPERTY_VALUES, null);
		if (setters.length > 0)
		{
	...
	private static void validate(final Class<?> target, final String[] getters, final String[] setters, final Class[] types,
			final Method[] getters_out, final Method[] setters_out)
	{
		int i = -1;
		...
		try
		{
			for (i = 0; i < types.length; i++)
			{
				if (getters[i] != null)
				{
					...
					{
						throw new BulkBeanException(
								"Specified type " + types[i] + " does not match declared type " + method.getReturnType(), i);
					}
				...
				if (setters[i] != null)
				{
					final Method method = ReflectUtils.findDeclaredMethod(target, setters[i], new Class[]
					{ types[i] });
					if (Modifier.isPrivate(method.getModifiers()))
					{
						throw new BulkBeanException("Property is private", i);
