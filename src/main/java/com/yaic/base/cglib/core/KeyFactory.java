...
/**
 * Generates classes to handle multi-valued keys, for use in things such as Maps and Sets. Code for <code>equals</code>
 * and <code>hashCode</code> methods follow the the rules laid out in <i>Effective Java</i> by Joshua Bloch.
 * <p>
 * To generate a <code>KeyFactory</code>, you need to supply an interface which describes the structure of the key. The
 * interface should have a single method named <code>newInstance</code>, which returns an <code>Object</code>. The
 * arguments array can be <i>anything</i>--Objects, primitive values, or single or multi-dimension arrays of either. For
 * example:
 * <p>
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * Once you have made a <code>KeyFactory</code>, you generate a new key by calling the <code>newInstance</code> method
 * defined by your interface.
 * <p>
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * <b>Note:</b> <code>hashCode</code> equality between two keys <code>key1</code> and <code>key2</code> is only
 * guaranteed if <code>key1.equals(key2)</code> <i>and</i> the keys were produced by the same factory.
 *
 */
public abstract class KeyFactory
{
	...
	private static final Type KEY_FACTORY = TypeUtils.parseType("com.yaic.base.cglib.core.KeyFactory");
	...
	{ 11, 73, 179, 331, 521, 787, 1213, 1823, 2609, 3691, 5189, 7247, 10037, 13931, 19289, 26627, 36683, 50441, 69403, 95401,
			131129, 180179, 247501, 340057, 467063, 641371, 880603, 1209107, 1660097, 2279161, 3129011, 4295723, 5897291, 8095873,
			11114263, 15257791, 20946017, 28754629, 39474179, 54189869, 74391461, 102123817, 140194277, 192456917, 264202273,
	...
	/**
	 * {@link Type#hashCode()} is very expensive as it traverses full descriptor to calculate hash code. This customizer
	 * uses {@link Type#getSort()} as a hash code.
	 */
	 ...
	 * @param customizer
	 * @param next
	 * @return the keyFactory the keyFactory
	 */
	public static KeyFactory create(final ClassLoader loader, final Class keyInterface, final KeyFactoryCustomizer customizer,
			final List<KeyFactoryCustomizer> next)
	{
	...
	public static class Generator extends AbstractClassGenerator
	{
		...
		private static final Class[] KNOWN_CUSTOMIZER_TYPES = new Class[]
		{ Customizer.class, FieldTypeCustomizer.class };
		...
		public void generateClass(final ClassVisitor v)
		{
			...
			final Method newInstance = ReflectUtils.findNewInstance(keyInterface);
			...
			ce.begin_class(Constants.V1_2, Constants.ACC_PUBLIC, getClassName(), KEY_FACTORY, new Type[]
			{ Type.getType(keyInterface) }, Constants.SOURCE_FILE);
			...
			final int hm = (multiplier != 0) ? multiplier : PRIMES[Math.abs(seed * 13) % PRIMES.length];
			e.push(hc);
			for (int i = 0; i < parameterTypes.length; i++)
			{
				e.load_this();
