...
import com.yaic.commons.io.LineIterator;
 ...
 * {@link #copy(InputStream, OutputStream, int)} which creates the buffer and calls
 * {@link #copyLarge(InputStream, OutputStream, byte[])}.
 * <p>
 * Applications can re-use buffers by using the underlying methods directly. This may improve performance for
 * applications that need to do a lot of copying.
 * <p>
...
public class IOUtil
{
	...
	/*
	 * These buffers are static and are shared between threads. This is possible because the buffers are write-only - the
	 * contents are never read.
	 *
	 * N.B. there is no need to synchronize when creating these because: - we don't care if the buffer is created
	 * multiple times (the data is ignored) - we always use the same size buffer, so if it it is recreated it will still
	 * be OK (if the buffer size were variable, we would need to synch. to ensure some other thread did not create a
	 * smaller one)
	 */
	 ...
	 * </ul>
	 * It can be used in favor of {@link #toByteArray(InputStream)}, since it avoids unnecessary allocation and copy of
	 * byte[].<br>
	 ...
	 * {@link IOUtil#toByteArray(java.io.InputStream, int)} to read into the byte array. (Arrays can have no more than
	 ...
	 * @return the requested byte array
	 * @throws IOException
	 *            if an I/O error occurs or <code>InputStream</code> size differ from parameter size
	 * @throws IllegalArgumentException
	 *            if size is less than zero or size is greater than Integer.MAX_VALUE
	 * @see IOUtil#toByteArray(java.io.InputStream, int)
	 */
	...
	{
		return toCharArray(is, StandardCharset.forName(encoding));
	}
	...
	{
		return toString(input, StandardCharset.forName(encoding));
	}
	...
	{
		return toString(uri, StandardCharset.forName(encoding));
	}
	...
	{
		return toString(url, StandardCharset.forName(encoding));
	}
	...
	{
		return new String(input, StandardCharset.forName(encoding));
	}
	...
	/**
	 * Gets the contents of an <code>InputStream</code> as a list of Strings, one entry per line, using the specified
	 * character encoding.
	 * <p>
	 * This method buffers the input internally, so there is no need to use a <code>BufferedInputStream</code>.
	 *
	...
	{
		return readLines(input, StandardCharset.forName(encoding));
	}
	 ...
	 * <p>
	 * <code>LineIterator</code> holds a reference to the open <code>Reader</code> specified here. When you have finished
	 * with the iterator you should close the reader to free internal resources. This can be done by closing the reader
	 * directly, or by calling {@link LineIterator#close()} or {@link LineIterator#closeQuietly(LineIterator)}.
	 * <p>
	...
	/**
	 * Returns an Iterator for the lines in an <code>InputStream</code>, using the character encoding specified (or
	 * default encoding if null).
	 * <p>
	 * <code>LineIterator</code> holds a reference to the open <code>InputStream</code> specified here. When you have
	 * finished with the iterator you should close the stream to free internal resources. This can be done by closing the
	 * stream directly, or by calling {@link LineIterator#close()} or {@link LineIterator#closeQuietly(LineIterator)}.
	 * <p>
	 * The recommended usage pattern is:
	 *
	 * <pre>
	...
	{
		return lineIterator(input, StandardCharset.forName(encoding));
	}
	...
	{
		return toInputStream(input, StandardCharset.forName(encoding));
	}
	...
	{
		final byte[] bytes = input.getBytes(StandardCharset.forName(encoding));
	...
	/**
	 * Writes bytes from a <code>byte[]</code> to an <code>OutputStream</code> using chunked writes. This is intended for
	 * writing very large byte arrays which might otherwise cause excessive memory usage if the native code has to
	 * allocate a copy.
	 *
	 * @param data
	 *           the byte array to write, do not modify during output, null ignored
	...
	{
		write(data, output, StandardCharset.forName(encoding));
	}
	...
	/**
	 * Writes chars from a <code>char[]</code> to a <code>Writer</code> using chunked writes. This is intended for
	 * writing very large byte arrays which might otherwise cause excessive memory usage if the native code has to
	 * allocate a copy.
	 *
	 * @param data
	...
	{
		writeLines(lines, lineEnding, output, StandardCharset.forName(encoding));
	}

	/**
	 * Writes the <code>toString()</code> value of each item in a collection to a <code>Writer</code> line by line, using
	...
	/**
	 * Copies some or all bytes from a large (over 2GB) <code>InputStream</code> to an <code>OutputStream</code>,
	 * optionally skipping input bytes.
	 * <p>
	 * This method buffers the input internally, so there is no need to use a <code>BufferedInputStream</code>.
	 * </p>
	 * <p>
	 * Note that the implementation uses {@link #skip(InputStream, long)}. This means that the method may be considerably
	 ...
	 *           the <code>OutputStream</code> to write to
	 * @param inputOffset
	 *           : number of bytes to skip from input before copying -ve values are ignored
	 * @param length
	...
	/**
	 * Copies bytes from an <code>InputStream</code> to chars on a <code>Writer</code> using the specified character
	 * encoding.
	 * <p>
	 * This method buffers the input internally, so there is no need to use a <code>BufferedInputStream</code>.
	 * <p>
	 ...
	 *           the <code>Writer</code> to write to
	 * @param inputEncoding
	 *           the encoding to use for the input stream, null means platform default
	 * @throws NullPointerException
	 *            if the input or output is null
	 * @throws IOException
	 *            if an I/O error occurs
	...
	{
		copy(input, output, StandardCharset.forName(inputEncoding));
	}
	...
	/**
	 * Copies some or all chars from a large (over 2GB) <code>InputStream</code> to an <code>OutputStream</code>,
	 ...
	 *           the <code>Writer</code> to write to
	 * @param inputOffset
	 *           : number of chars to skip from input before copying -ve values are ignored
	 * @param length
	...
	/**
	 * Copies chars from a <code>Reader</code> to bytes on an <code>OutputStream</code> using the specified character
	...
	{
		copy(input, output, StandardCharset.forName(outputEncoding));
	}
	 ...
	 * before giving up; this may not always be the case for skip() implementations in subclasses of {@link InputStream}.
	 * <p>
	 * Note that the implementation uses {@link InputStream#read(byte[], int, int)} rather than delegating to
	 * {@link InputStream#skip(long)}. This means that the method may be considerably less efficient than using the
	 * actual skip implementation, this is done to guarantee that the correct number of bytes are skipped.
	 * </p>
	 *
	 * @param input
	 *           byte stream to skip
	...
	/**
	 * Skips bytes from a ReadableByteChannel. This implementation guarantees that it will read as many bytes as possible
	 * before giving up.
	 *
	 * @param input
	 ...
	 * of {@link Reader}.
	 * <p>
	 * Note that the implementation uses {@link Reader#read(char[], int, int)} rather than delegating to
	 * {@link Reader#skip(long)}. This means that the method may be considerably less efficient than using the actual
	 ...
	 * likely because of reaching EOF).
	 * <p>
	 * Note that the implementation uses {@link #skip(Reader, long)}. This means that the method may be considerably less
	 * efficient than using the actual skip implementation, this is done to guarantee that the correct number of
	 * characters are skipped.
	 * </p>
	 *
	 * @param input
	...
	/**
	 * Reads characters from an input character stream. This implementation guarantees that it will read as many
	 ...
	 * <p>
	 * This implementation guarantees that it will read as many bytes as possible before giving up; this may not always
	 * be the case for subclasses of {@link ReadableByteChannel}.
	 *
	 * @param input
	 ...
	 * <p>
	 * This allows for the possibility that {@link Reader#read(char[], int, int)} may not read as many characters as
	 ...
	 * <p>
	 * This allows for the possibility that {@link InputStream#read(byte[], int, int)} may not read as many bytes as
	 * requested (most likely because of reaching EOF).
	 *
	 * @param input
	 *           where to read input from
	 ...
	 * Reads the requested number of bytes or fail if there are not enough left.
	 * <p>
	 * This allows for the possibility that {@link ReadableByteChannel#read(ByteBuffer)} may not read as many bytes as
	 * requested (most likely because of reaching EOF).
	 *
	 * @param input
	 *           the byte channel to read
